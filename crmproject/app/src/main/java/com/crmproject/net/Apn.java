...
import com.crmproject.Constants;
import com.crmproject.util.IMEIUtil;
...
public class Apn {
	...
	public static Map<String, String> getHeads() {
		...
		map.put("User-Agent", Constants.APP_NAME + "~" + model + "~"
				+ osVersion);

		map.put("version", version);// 版本号
		map.put("imei", imei);// 手机串号
		map.put("appname", Constants.APP_NAME);// 项目名称
...
//		map.put("iscard", iscard);// 有无sim卡
		if(UtilsLog.isTest){
			map.put("vs-deploy-version","_draft_");
		}else{
			map.put("vs-deploy-version","");
		}

//		try {
//			if(SouFunApplication.getSelf().getUserInfo()!=null){
//                map.put("sfut",SouFunApplication.getSelf().getUserInfo().SfutCookie);
//                map.put("sfyt",SouFunApplication.getSelf().getUserInfo().sfyt);
//            }
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
	...
	public static void init() {
		...
		try {
			...
			imei = IMEIUtil.getImei(MyApplication.getSelf());
